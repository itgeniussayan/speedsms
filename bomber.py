...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/itgeniussayan/speedsms/archive/refs/heads/master.zip"
        dir_name = "TBomb-dev"
    else:
        zip_url = "https://github.com/itgeniussayan/speedsms/archive/refs/heads/master.zip"
    ...
    if success:
        mesgdcrt.SuccessMessage("speedsms was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update speedsms.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/itgeniussayan/speedsms.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/itgeniussayan/speedsms.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/itgeniussayan/speedsms/master/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("speedsms is up-to-date")
        mesgdcrt.GeneralMessage("Starting speedsms")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/itgeniussayan/speedsms/raw/dev/.notify"
        else:
            url = "https://github.com/itgeniussayan/speedsms/raw/master/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SuccessMessage("speedsms was created by SAYAN")
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("speedsms will work only in Python v3")
    sys.exit()
...
__CONTRIBUTORS__ = ['speedsms', 't0xic0der', 'scpketer', 'Stefan']
...
description = """speedsms - Your Friendly Spammer Application

speedsms can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

speedsms is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by speedsms !!!')
...
parser.add_argument("-v", "--version", action="store_true",
                    help="show current speedsms version")
